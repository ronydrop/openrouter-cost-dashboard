...
const colorMap = {
  ...
  critical: 'bg-red-50 dark:bg-red-900/30 border-red-500 text-red-800 dark:text-red-200',
};

const severityBadgeMap = {
  info: 'bg-blue-100 text-blue-700 dark:bg-blue-800 dark:text-blue-200',
  warning: 'bg-amber-100 text-amber-700 dark:bg-amber-800 dark:text-amber-200',
  critical: 'bg-red-100 text-red-700 dark:bg-red-800 dark:text-red-200',
};
  ...
  if (loading) {
    return (
      <div className="space-y-3">
        {[...Array(3)].map((_, i) => (
          <div key={i} className="bg-white dark:bg-slate-800 rounded-lg shadow p-4 animate-pulse">
  ...
  if (!insights?.length) {
    return (
      <div className="bg-white dark:bg-slate-800 rounded-lg shadow p-8 text-center">
        <Lightbulb className="mx-auto text-gray-300 dark:text-gray-600 mb-3" size={40} />
        <p className="text-gray-500 dark:text-gray-400">Nenhum insight disponível para o período</p>
        <p className="text-sm text-gray-400 dark:text-gray-500 mt-2">Sincronize os dados para gerar insights</p>
      </div>
    );
  }
    ...
    <div className="space-y-3">
      ...
      {insights.map((insight) => {
        ...
        return (
          <div
            key={insight.id}
            className={`bg-white dark:bg-slate-800 rounded-lg shadow border-l-4 p-4 ${colorClass}`}
          >
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                {insight.meta && Object.keys(insight.meta).length > 0 && (
                  <div className="mt-2 p-2 bg-gray-50 dark:bg-slate-700/50 rounded-lg text-xs text-gray-600 dark:text-gray-400">
                    ...
                    {insight.meta.changePercent !== undefined && (
                      <span className={insight.meta.changePercent > 0 ? 'text-red-600 dark:text-red-400' : 'text-green-600 dark:text-green-400'}>
